...
async function analyzeUser(username: string): Promise<AnalysisResult> {
  const [user, repos, events] = await Promise.all([
    fetchGitHubUser(username), fetchGitHubRepos(username), fetchGitHubEvents(username),
  ]);
...
export async function GET(request: NextRequest) {
  ...
  if (!user1 || !user2) return NextResponse.json({ error: "Two usernames are required" }, { status: 400 });

  const clean1 = user1.trim().replace(/[^a-zA-Z0-9-]/g, "");
  const clean2 = user2.trim().replace(/[^a-zA-Z0-9-]/g, "");
  if (!clean1 || !clean2) return NextResponse.json({ error: "Invalid usernames" }, { status: 400 });

  try {
    const [result1, result2] = await Promise.all([analyzeUser(clean1), analyzeUser(clean2)]);
