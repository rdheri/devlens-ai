  ...
  return (
    <div className="card-gradient p-6 flex flex-col items-center justify-center">
      ...
      <p className="text-xs text-txt-muted mt-3 text-center">
        Weighted average across all skill dimensions
      </p>
    </div>
  );
}
    ...
    return (
      <div className="min-h-screen grid-bg flex items-center justify-center px-6">
        ...
        <div className="relative z-10 text-center max-w-md">
          ...
          <button onClick={() => router.push("/")} className="px-6 py-2.5 bg-accent-cyan/10 text-accent-cyan font-semibold text-sm rounded-xl border border-accent-cyan/20 hover:bg-accent-cyan/20 transition-all">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <nav className="flex items-center justify-between mb-8 animate-fade-up">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => router.push(`/compare?prefill=${user.login}`)}
              className="text-[10px] font-mono text-txt-muted hover:text-accent-violet transition-colors"
            >
              Compare ⬡
            </button>
          ...
          <div className="flex flex-col sm:flex-row items-start gap-6">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-baseline gap-3 mb-1">
                ...
                <a href={user.html_url} target="_blank" rel="noopener noreferrer" className="text-sm font-mono text-txt-muted hover:text-accent-cyan transition-colors">
                  @{user.login} ↗
                </a>
              </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                {analysis.strengths.slice(0, 3).map((s) => (
                  <span key={s} className="px-2.5 py-1 rounded-lg bg-surface-3 border border-surface-4 text-txt-secondary text-xs">{s}</span>
                ))}
              </div>
        ...
        {/* Main Grid: Overall Score + Radar + Languages */}
        ...
        <section className="card-gradient p-6 mb-8 animate-fade-up" style={{ animationDelay: "0.4s" }}>
          ...
          <p className="text-xs text-txt-muted mb-4">Personalized growth roadmap based on profile analysis</p>
        ...
        <footer className="text-center pb-8 space-y-3">
          <div className="flex justify-center gap-3">
            <button onClick={() => router.push("/")} className="px-6 py-2.5 bg-surface-2 text-txt-secondary text-sm rounded-xl border border-surface-4 hover:border-accent-cyan/30 hover:text-accent-cyan transition-all">
              Analyze Another →
            </button>
            <button onClick={() => router.push("/compare")} className="px-6 py-2.5 bg-accent-violet/10 text-accent-violet text-sm rounded-xl border border-accent-violet/20 hover:bg-accent-violet/20 transition-all">
              Compare Developers ⬡
            </button>
          </div>
