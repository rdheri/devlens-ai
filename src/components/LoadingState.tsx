...
const PHASES = [
  ...
  { msg: "Scanning file trees & README content...", icon: "⬡" },
  { msg: "Detecting tests, CI/CD, Docker configs...", icon: "◈" },
...
export default function LoadingState({ username }: { username: string }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => setPhase((p) => (p < PHASES.length - 1 ? p + 1 : p)), 2500);
      ...
      <div className="relative z-10 text-center max-w-md">
        <div className="relative w-24 h-24 mx-auto mb-8">
          ...
          <div className="absolute inset-4 rounded-full bg-surface-2 border border-accent-cyan/20 flex items-center justify-center">
            <span className="text-accent-cyan text-xl animate-pulse">{PHASES[phase].icon}</span>
          </div>
        </div>
        <h2 className="font-display font-700 text-xl text-txt-primary mb-2">
          Analyzing <span className="text-accent-cyan font-mono">{username}</span>
        </h2>
        <div className="h-6 mb-6"><p className="text-sm text-txt-secondary animate-fade-in" key={phase}>{PHASES[phase].msg}</p></div>
        <div className="w-full h-1 bg-surface-3 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-accent-cyan to-accent-emerald rounded-full transition-all duration-1000 ease-out" style={{ width: `${((phase + 1) / PHASES.length) * 100}%` }} />
        </div>
